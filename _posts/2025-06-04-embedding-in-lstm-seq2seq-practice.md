...
为了更科学地验证embedding的学习效果，脚本中集成了全面的**质量分析系统**，在生成可视化图之前自动执行：

#### 🔍 词汇相似度分析

系统自动计算所有词汇对的余弦相似度，并输出最相似的词汇对：
...
这些数值验证了可视化图中观察到的聚集现象。

#### 🎯 主题词汇聚集度分析

系统针对预定义的主题群体计算**组内平均相似度**：
...
这验证了**上下文学习效果**：一起出现的词汇确实学到了更相似的表示。

#### 📊 统计特性分析

系统还提供embedding的整体统计信息：

```
📊 Embedding统计特性:
   向量范数 - 均值: 2.1234, 标准差: 0.3456
   相似度分布 - 均值: 0.1234, 标准差: 0.2345
   相似度范围: [-0.2345, 0.8734]
```

#### ⚠️ 异常向量检测

系统自动检测是否存在异常的embedding向量：
- 范数过大或过小的向量
- 与其他向量相似度异常的词汇
- 可能的训练问题指示器

这个定量验证系统与可视化图像相互印证，提供了**多维度的embedding质量评估**，确保我们对模型学习效果的判断建立在科学数据基础之上。
