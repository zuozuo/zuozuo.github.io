...
但是上面的程序需要将所有的回文数放入数组中再取最大，这就增加了  
空间复杂度：`O(arr.length) = O(1239)` 和 时间复杂度：遍历数组取最大元素`O(a.length) = O(1239)`  
那有没有更好的方法呢？  
观察之后我们会发现我们的两层循环在遍历三位数乘积的时候同时也是对所有的回文数做了一边遍历，我们可以直接在第一次遍历的时候就取出最大元素：
